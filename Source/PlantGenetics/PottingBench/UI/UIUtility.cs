...
namespace PlantGenetics
{
    public static class UIUtility
    {
        ...
        public static void DrawCountAdjuster(ref int value, Rect inRect, ref string buffer, int min, int max,
            bool readOnly = false, int? setToMin = null, int? setToMax = null)
        {
            var temp = value;
            ...
            if (!readOnly && (setToMin.HasValue ? value > setToMin.Value : value != min) &&
                Widgets.ButtonText(leftBigRect, "<<")) value = setToMin ?? min;
            if (!readOnly && value - mult >= min && Widgets.ButtonText(leftSmallRect, "<")) value -= mult;
            if (!readOnly && (setToMax.HasValue ? value < setToMax.Value : value != max) &&
                Widgets.ButtonText(rightBigRect, ">>")) value = setToMax ?? max;
